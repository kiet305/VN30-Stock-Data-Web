...
from .market_data_validator import build_verified_market_snapshot as get_yfinance_market_snapshot
from .warehouse_market import (
    get_stock_data as get_warehouse_stock,
    get_indicator as get_warehouse_indicator,
    build_verified_market_snapshot as get_warehouse_market_snapshot,
)
...
TOOLS_CATEGORIES = {
    ...
    "market_data_validation": {
        "description": "Verified market snapshots",
        "tools": [
...
VENDOR_METHODS = {
    ...
    # technical_indicators
    "get_indicators": {
        "warehouse": get_warehouse_indicator,
        "alpha_vantage": get_alpha_vantage_indicator,
        "yfinance": get_stock_stats_indicators_window,
    },
    "get_verified_market_snapshot": {
        "warehouse": get_warehouse_market_snapshot,
...
def route_to_vendor(method: str, *args, **kwargs):
    ...
    vendor_config_text = str(vendor_config or "default").strip()
    primary_vendors = [v.strip() for v in vendor_config_text.split(',') if v.strip()]

    if method not in VENDOR_METHODS:
        raise ValueError(f"Method '{method}' not supported")

    # Build fallback chain: primary vendors first, then remaining available vendors.
    # A single explicit vendor (for example "warehouse") is treated as strict so
    # schema/connectivity problems are surfaced instead of being hidden by a
    # noisy public-data fallback.
    all_available_vendors = list(VENDOR_METHODS[method].keys())
    fallback_vendors = primary_vendors.copy()
    should_append_fallbacks = vendor_config_text.lower() == "default" or "," in vendor_config_text
    if should_append_fallbacks:
