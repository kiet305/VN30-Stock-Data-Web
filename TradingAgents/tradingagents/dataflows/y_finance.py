...
def get_stock_stats_indicators_window(
    ...
    best_ind_params = {
        ...
        "obv": (
            "OBV: On-Balance Volume is a cumulative signed-volume measure. "
            "Usage: Confirm whether accumulation supports price advances or distribution undermines rallies. "
            "Tips: Interpret the slope and divergence rather than the absolute number."
        ),
        "cmf": (
            "CMF: 20-period Chaikin Money Flow uses close location within the high-low range and volume. "
            "Usage: Positive values suggest accumulation; negative values suggest distribution. "
            "Tips: Combine with trend filters to avoid noisy signals."
        ),
        "volume_20_sma": (
            "Volume 20 SMA: Average daily volume over the last 20 trading rows. "
            "Usage: Establish the short-term liquidity baseline."
        ),
        "volume_50_sma": (
            "Volume 50 SMA: Average daily volume over the last 50 trading rows. "
            "Usage: Establish the medium-term liquidity baseline."
        ),
        "volume_ratio_20": (
            "Relative Volume 20D: Current volume divided by 20-day average volume. "
            "Usage: Identify unusually high or low participation."
        ),
        "volume_zscore_20": (
            "Volume Z-Score 20D: Current volume measured in 20-day standard deviations from average. "
            "Usage: Detect statistically unusual volume spikes or dry-ups."
        ),
    }
...
def _get_stock_stats_bulk(
    ...
    _ensure_custom_volume_indicator(df, indicator)
...
def _series(df: pd.DataFrame, *names: str) -> pd.Series:
    ...
    raise KeyError(f"None of these columns are available: {names}")


def _ensure_custom_volume_indicator(df: pd.DataFrame, indicator: str) -> None:
    if indicator not in {
        "obv",
        "cmf",
        "volume_20_sma",
    ...
    volume = _series(df, "volume", "Volume")
    nan = float("nan")

    df["volume_20_sma"] = volume.rolling(20, min_periods=1).mean()
    df["volume_50_sma"] = volume.rolling(50, min_periods=1).mean()
    df["volume_ratio_20"] = volume / df["volume_20_sma"].replace(0, nan)
    vol20_std = volume.rolling(20, min_periods=2).std()
    df["volume_zscore_20"] = (volume - df["volume_20_sma"]) / vol20_std.replace(0, nan)
    ...
    money_flow_multiplier = (
        ...
        / (high - low).replace(0, nan)
    ).fillna(0)
    money_flow_volume = money_flow_multiplier * volume.fillna(0)
    df["cmf"] = (
        money_flow_volume.rolling(20, min_periods=1).sum()
        / volume.fillna(0).rolling(20, min_periods=1).sum().replace(0, nan)
    )
